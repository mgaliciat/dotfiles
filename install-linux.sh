...
# máquina recién clonada cae al default versionado (carbon). Sembramos el baseline
# para paridad de look. Acá solo aplican nvim + tmux (sin ghostty en Linux/WSL2);
# scripts/theme omite la capa Ghostty sin error. Set-if-absent: no pisa una
# elección local hecha con `scripts/theme`. Cambiar baseline = editar DEFAULT_THEME.
DEFAULT_THEME="osaka"
