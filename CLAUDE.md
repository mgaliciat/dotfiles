...
**Versioned skills are per-item symlinks, never the parent dir.** `claude/skills/logbook/` is a skills-dir plugin (`.claude-plugin/plugin.json`), loaded in place — a `git pull` propagates edits with no marketplace and no copy into `~/.claude/plugins/cache/`. It therefore lives in `settings.sh`, not `plugins.sh`. Plugin skills are **always** colon-namespaced: `/logbook:entry`, `/logbook:ingest`, `/logbook:query`, `/logbook:lint`, `/logbook:guide`, `/logbook:runbook`, `/logbook:document`, `/logbook:walkthrough`, `/logbook:task`, where the folder name is the skill name — so the folders are bare (`entry`, not `wiki-lint`, which once produced `/wiki:wiki-lint`).

**`claude/skills/team/` is the other versioned skill, and it is a PLAIN one** — a single `SKILL.md`, so it invokes bare as `/team`, with none of the colon namespacing a plugin forces. It orchestrates agent teams and is the documentation for the observers; the two features are wired together, so read the next section before touching either.

**`claude/agents/` holds user-level agent definitions**, symlinked per-item into `~/.claude/agents/` by `settings.sh`. Three today: `scope-guard` and `regression-watch` (background observers) and `teammate-base` (the role `/team` spawns teammates with). Unlike `claude/skills/*`, this dir is **not** gitignored — there is no whitelist to edit when adding one, only the loop in `settings.sh` and its Windows twin.
...
|---|---|---|---|---|
| **1** | `settings.sh` | **We do**, with `jq` | Our guard (only if the key is absent) | `statusLine`, `permissions.*`, `attribution.*`, `outputStyle`, `env.*`, the logbook `PostToolUse` hook, stale-hook cleanup, symlinks for `statusline.sh` + `CLAUDE.md` + `hooks/logbook.sh` + the `logbook` skill plugin + the `team` skill + `claude/agents/*` |
...
- **The agent-team and observer env vars only take effect at STARTUP.** `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` is what puts the `name` parameter on the Agent tool, and that schema is fixed when the session starts — so a session already running when the installer wrote the key has no teams and says nothing about it. The check is the tool's own schema, not the settings file. Both flags are additionally gated server-side, and neither does anything under `-p`.
- **`CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS` is the ceiling those two need.** The harness default is 20; we set **6**. It counts every concurrent agent — teammates, their observers, and any subagent a teammate spawns — and it is genuinely enforced: past it the Agent call is refused with *"Concurrent subagent limit reached. Do not retry."* The cap is **global**, not team-scoped, so raising or lowering it changes ordinary fan-out in every session too. `CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION` looks like its sibling, appears in the binary's env-name registry, and has **no reader** — setting it is a silent no-op.
- **`observer:` / `observerMessage:` / `observeSubagents:` are undocumented frontmatter.** Verified in the 2.1.267 binary; on no docs page. An agent-definition field the loader doesn't recognise is ignored in silence, so when an observer stops appearing, check the binary before rewriting the file. `teammate-base` sets `observeSubagents: false` on purpose — inherited observers multiply down the spawn chain and eat the cap above. An observer never gets its own observer, and observer fan-out stops at depth 2, hardcoded.
- **A teammate ignores the `skills:` frontmatter field.** For an ordinary subagent it preloads the full skill content; for a teammate it does nothing and the teammate loads project/user skills the normal way. So a role that lists `skills:` and a spawn prompt that never names one produces a teammate that runs no skill, with nothing in the output saying so — `/team` names the skill in the spawn prompt for exactly this reason.
