...
- **Sync Ghostty ↔ nvim para los themes Anthropic.** El bloque inline DARK/LIGHT en `ghostty/config.ghostty` (sección `# ─── tema (Anthropic custom inline)`) y los módulos `nvim/lua/themes/anthropic-dark.lua` / `anthropic-light.lua` comparten paleta hex. Si tocás un color en un lado, replicar en el otro a mano. Los themes legacy de nvim (`obsidian`, `warm`, `paper`) son self-contained — no requieren sync.
