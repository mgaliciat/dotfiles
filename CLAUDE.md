...
**Excepción controlada — merge aditivo de hooks.** `install.sh` (y `install-linux.sh`) SÍ tocan el `settings.json` real, pero **sin symlinkear ni clobberear**: hacen un *merge idempotente con `jq`* del fragmento versionado `claude/claude-session-hooks.json` (los hooks de estado de `tmux-claude-session-manager`). El fragmento es seguro de versionar porque es config genérica del plugin, **cero estado personal**. El merge appendea los hooks por-evento preservando el resto del `settings.json` y cualquier `.hooks` que ya tengas; un guard (`jq -e ... any(test("…/state.sh"))`) garantiza idempotencia (no duplica en re-runs) y la escritura es atómica (`mktemp` + `mv`). El plugin se pre-clona en el mismo bloque para que `state.sh` exista antes del merge (sino los hooks darían exit 127). El merge es **convergente**: borrar los hooks a mano del `settings.json` NO persiste — el próximo `install.sh` los re-agrega. Para optar por no tenerlos en una máquina, remové el bloque del installer o el fragmento localmente (no corras ese paso). Fuera de tmux igual son inofensivos: `state.sh` sale 0 por `$TMUX_PANE` vacío.
...
- **Familia canónica (10):** `dark-2026` (clon de "Dark 2026", el theme oscuro por default de VS Code desde 1.113 — hex sacados de `extensions/theme-defaults/themes/2026-dark.json` en microsoft/vscode, **default del stack**), `light-2026` (companion claro — clon de "2026 Light", `extensions/theme-defaults/themes/2026-light.json`), `carbon` (minimal true-black, high contrast, acento Claude orange), `osaka` (deep-ocean craftzdog), `xcode-oled` (true black #000 + syntax de Xcode "Default (Dark)" — hex literales del plist `Default (Dark).xccolortheme` de Apple, verificados contra dos copias independientes; los ports a VS Code **no** son fieles y no sirven de fuente. Reemplazó a `oled-neon`), `anthropic-dark`, `anthropic-warm`, `prism-night`, `paper`, `solarized-light` (Solarized Light canónico). Alias osaka (sub-sabores del plugin, no entradas de la matriz): `solarized-osaka`, `osaka-moon`, `osaka-storm`, `osaka-day`. `obsidian` sigue siendo theme nvim válido pero queda **fuera** del switcher (solo nvim).
- **Definiciones Y selección versionadas.** Cada tema tiene su paleta en `ghostty/themes/<id>`, `nvim/lua/themes/<id>.lua` (excepto osaka, que es el plugin `solarized-osaka.nvim`) y `tmux/themes/<id>.conf`. La *selección activa* son tres punteros **versionados** que escribe el switcher: `ghostty/themes/current`, `nvim/lua/theme-current`, `tmux/theme-current.conf`. Toggling **SÍ ensucia el working tree**: hay que commitear para que un `git pull` propague el tema a las otras máquinas. Este es el punto — los punteros eran `*.local` (gitignored) y el tema nunca viajaba; cambiar el `theme =` de un config y pullear no hacía nada, porque el puntero local seguía ganando. Si querés que una máquina tenga su propio tema, gitignorá su puntero.
- **Cada capa lee el puntero como override-at-end**: Ghostty `config-file = ?~/.config/ghostty/themes/current` (último `theme=` gana; `?` = opcional); nvim lee `theme-current` en `options.lua` con fallback al default; tmux `source -q theme-current.conf` después de la paleta default. Los defaults hardcodeados en los tres configs son sólo red de seguridad por si borrás el puntero → todo cae a dark-2026.
- **Los punteros llegan por los symlinks de dir.** `~/.config/{ghostty/themes,nvim,tmux}` son symlinks al repo, así que un `git pull` entrega el puntero nuevo sin re-correr `install.sh`. Por eso los installers **no** siembran ningún tema baseline — ese bloque existió y se removió al versionar la selección.
...
- **`tmux-claude-session-manager` (craftzdog) — picker de sesiones Claude.** Declarado en `tmux.conf` (`@plugin`), pero `install.sh`/`install-linux.sh` lo **pre-clonan** además de tpm: el merge de hooks necesita su `scripts/state.sh` presente antes de correr (ver "merge aditivo de hooks" arriba). El plugin bindea sus comandos en la tabla **prefix** (prefix+y launcher, prefix+u picker), pero los **rebindeamos a chord único `Alt+y`/`Alt+u`** para que matcheen el resto de popups de `utility.conf`: los `bind -n M-y/M-u` (que llaman a los scripts `launch.sh`/`list.sh` del plugin) viven en `utility.conf`, y los de prefix se desactivan con `unbind y`/`unbind u` en `tmux.conf` **después** del `run` de tpm (sino el plugin los re-bindea encima). Entonces: `Alt+u` = picker central de TODAS las sesiones Claude (estado live working/waiting/idle + preview), `Alt+y` = launcher por directorio. El estado live depende de los 4 hooks en `settings.json`; sin ellos el picker igual lista/preview/jump/kill, solo muestra `?`. Convive **sin conflicto** con el popup casero `Alt+c`: el `session_hash()` del plugin está hecho a propósito para igualar `echo path | md5sum | cut -c1-8` — el mismo algoritmo del `Alt+c` —, así que para un mismo dir `Alt+c` y `Alt+y` resuelven a la **misma** sesión `claude-<hash>` y la comparten (no se duplican). El `Alt+u` del plugin **reemplazó al viejo `Alt+s`** (selector per-proyecto, ya retirado junto con `scripts/tmux-claude`): el picker es un superset (todas las sesiones, estado live, preview). El picker matchea por prefijo `claude-`, así que también lista las sesiones que creó `Alt+c`. El `Alt+C` YOLO sí es sesión aparte (prefijo `claude-yolo-`).
