...
- **`tmux-claude-session-manager` (craftzdog) — picker de sesiones Claude.** Declarado en `tmux.conf` (`@plugin`), pero `install.sh`/`install-linux.sh` lo **pre-clonan** además de tpm: el merge de hooks necesita su `scripts/state.sh` presente antes de correr (ver "merge aditivo de hooks" arriba). Da `prefix + u` (picker central de TODAS las sesiones Claude, con estado live working/waiting/idle + preview) y `prefix + y` (launcher por directorio). El estado live depende de los 4 hooks en `settings.json`; sin ellos el picker igual lista/preview/jump/kill, solo muestra `?`. Convive **sin conflicto** con el popup casero `Alt+c` de `utility.conf`: el `session_hash()` del plugin está hecho a propósito para igualar `echo path | md5sum | cut -c1-8` — que es el mismo algoritmo del `Alt+c` —, así que para un mismo dir `Alt+c` y `prefix+y` resuelven a la **misma** sesión `claude-<hash>` y la comparten (no se duplican). El `prefix+u` del plugin **reemplazó al viejo `Alt+s`** (selector per-proyecto, ya retirado junto con `scripts/tmux-claude`): el picker es un superset (todas las sesiones, estado live, preview). El picker matchea por prefijo `claude-`, así que también lista las sesiones que creó `Alt+c`. El `Alt+C` YOLO sí es sesión aparte (prefijo `claude-yolo-`).
